    ...
    public generateFooter(): NodeInterface[] | undefined {
        ...
        if (core.generator || core.datestamp || core.sourceLink || core.sourceUrl) {
            ...
            if (core.sourceLink && settings._source || core.sourceUrl) {
                ...
                text.push(new nodes.reference("", "View document source", [], {
                    refuri: source
                }), new nodes.Text(".\n"));
            }
            ...
            if (core.generator) {
                text.push(new nodes.Text("Generated by "), new nodes.reference("", "Docutils", [], {
                    refuri: "http://docutils.sourceforge.net/"
                }), new nodes.Text(" from "), new nodes.reference("", "reStructuredText", [], {
...
class Messages extends Transform {
    public apply(): void {
        ...
        const messages = unfiltered.filter(
            (msg: NodeInterface): boolean => msg.attributes.level >= threshold && msg.parent == null
        );

        if (messages.length) {
            const section = new nodes.section("", [], {
                classes: "system-messages"
            });

            // @@@ get this from the language module?
            section.children.push(new nodes.title("", "Docutils System Messages"), ...messages);
...
class FilterMessages extends Transform {
    public apply() {
        this.document.traverse({
            condition: nodes.system_message
        }).forEach((node, i) => {
