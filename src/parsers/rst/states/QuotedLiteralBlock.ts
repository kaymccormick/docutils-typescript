...
class QuotedLiteralBlock extends RSTState {
    ...
    public eof(context) {
        if (context.length) {
            const [src, srcline] = this.rstStateMachine.getSourceAndLine(this.initial_lineno);
            ...
            this.parent!.add(this.reporter!.warning("Literal block expected; none found.", [], {
    ...
    public indent(match: any, context: any[], nextState: any): any[] {
        ...
        //                         'be empty!')
        this.messages.push(this.reporter!.error("Unexpected indentation.", [], {
            line: this.rstStateMachine.absLineNumber()
        }));
    ...
    public initial_quoted(match, context, nextState) {
        ...
        this.addTransition("quoted", [pattern, this.quoted.bind(this), this.constructor.name]);
    ...
    public text(match, context, nextState) {
        if (context.length) {
            this.messages.push(this.reporter!.error("Inconsistent literal block quoting.", [], {
                line: this.rstStateMachine.absLineNumber()
            }));
