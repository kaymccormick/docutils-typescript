/** @uuid f2b03a34-3eff-4e77-a726-5abc3c76830e
*/
...
class Text extends RSTState {
    ...
    public blank(match: any, context: any[], nextState: any) {
        const [paragraph, literalNext] = this.paragraph(context, this.rstStateMachine.absLineNumber() - 1);
    ...
    public quoted_literal_block() {
        ...
        const newAbsOffset = this.nestedParse(
            ...
            parentNode,
            false,
            () => NestedStateMachine.createStateMachine(
                this.rstStateMachine,
                "QuotedLiteralBlock",
                this.rstStateMachine.stateFactory!.withStateClasses(["QuotedLiteralBlock"])
            )
        );
    ...
    public definition_list_item(termline: string[]): [NodeInterface, boolean] {
        ...
        const itemnode = new nodes.definition_list_item([...termline, ...indented].join("\b"));
            ...
            if (node instanceof nodes.Text) {
                ...
                if (parts.length === 1) {
                    ...
                    parts.slice(1).forEach((part): void => {
                        nodeList.push(new nodes.classifier(unescape(part, false), part));
                    });
                }
            } else {
                nodeList[nodeList.length - 1].add(node);
