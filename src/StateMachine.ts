...
class StateMachine implements Statemachine {
    ...
    states: States = {};
    inputLines: StringList = new StringList([]);

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    debugFn: DebugFunction = (line): void => {};
    ...
    public constructor(args: StateMachineConstructorArgs) {
        const cArgs = {
            ...args
        };
    ...
    public createStateMachine(
        rstStateMachine: RSTStateMachine,
        initialState?: string,
        stateFactory: Statefactory | undefined = rstStateMachine.stateFactory
    ): Statemachine {
        // @ts-ignore
            ...
            while (true) {
                try {
                    ...
                    if (error instanceof TransitionCorrection) {
                        ...
                        if (this.debug) {
                            this.debugFn(
                                `\nStateMachine.run: TransitionCorrection to ` + `state "${state.stateName}", transition ${transitions[0]}.`
                            );
                        }
                        ...
                        if (this.debug) {
                            this.debugFn(
                                `\nStateMachine.run: StateCorrection to state ` + `"${nextState}", transition ${tstr}.`
                            );
                        }
                    } else {
                        throw error;
    ...
    public getState(nextState?: string): StateInterface {
        if (nextState) {
            if (this.debug && nextState !== this.currentState) {
                this.debugFn(
                    `StateMachine.getState: ` + `changing state from "${this.currentState}"` + `to "${nextState}" (input line ${this.absLineNumber()})`
                );
            }
